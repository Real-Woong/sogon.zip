...
--
-- ⚠️ 지금 places.price_level은 1,364건 전부 NULL이라 이 값으로 거를 수 있는
-- 장소가 하나도 없다(05-open-questions Q14). 그래도 받아두는 이유는 코스
-- 카드에 "1인 8만원 예상"을 보여주고, 나중에 가격 데이터가 들어왔을 때
-- 소급할 수 없는 "그때 얼마로 잡았나"를 지금부터 남기기 위해서다.
-- **거르는 척은 하지 않는다.**
