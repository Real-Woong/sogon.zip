...
| 마이그레이션 | 0001, 0002, 0003 적용됨. **0004는 코드 완료, 프로덕션 미적용** (프리뷰는 #12에 따라 뒤처진 상태) |
...
1. [x] **`0004` — 날짜를 1급 객체로 (`date_plans`)** — 코드·로컬 스키마 검증 완료.
   **프로덕션 D1 적용은 아직 하지 않았다.**
...
2. [x] **데이트 약속 등록 + 상대에게 보이기** — `/api/date-plans` + 웹 화면.
   가장 작은 완결 기능이고, 혼자서도 리텐션이 생긴다. 상대 홈에 바로 뜬다.
...
4. **다음: `POST /api/recommendations/generate`** — 하드 필터 → 후보 생성 → 규칙 점수
