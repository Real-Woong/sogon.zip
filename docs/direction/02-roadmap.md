...
1. **API 키 발급** — 서울 열린데이터광장 → 공공데이터포털(TourAPI) → 카카오.
   진웅만 할 수 있다. 키는 저장소에 넣지 않는다 (`.dev.vars`, gitignore).
2. **수집 스크립트** — `scripts/ingest/`. 서울 문화행사부터.
   **로컬 node 스크립트로 돌려 원격 D1에 적재한다.** Cron Worker는 나중에.
   Pages Functions는 cron을 못 돌리고, 요청당 10ms 제한도 수집에 안 맞는다.
3. **지역 필터** — 상권 6곳 바깥은 넣지 않는다. `area_code`를 좌표로 채운다.
