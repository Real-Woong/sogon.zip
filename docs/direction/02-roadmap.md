...
**순서를 정한 근거는 `03-decisions.md` #18·#19에 있다.**
사람 쪽 데이터(취향)보다 장소 쪽 속성이 먼저다. 지금 취향을 물어봐도 매칭할
필드가 없다 — 음식점 458 + 카페 107이 태그 `"음식점"` 하나를 공유한다.

1. **카카오 로컬 수집기** ← **진웅: 카카오맵 제품 활성화**가 먼저다.
   지금은 403 `disabled OPEN_MAP_AND_LOCAL service`.
   값어치는 장소 개수가 아니라 **속성**이다. `category_name`(`음식점 > 한식 >
   육류,고기`)이 곧 음식 축이고, 지금 그 축이 통째로 비어 있다.
   수집기를 짜기 전에 카카오 약관의 저장·표시 제한을 확인하고 ADR에 남긴다.
2. **`POST /api/recommendations/generate`** — 하드 필터 → 후보 생성 → 규칙 점수 → 코스 조합
   → **impression 로깅**. 로깅이 이 엔드포인트의 절반이다.
   **카카오에 의존하지 않는다.** 종류 × 실내외 × 장르 × 무료 × 지역 × 기간이면
   "전시 + 카페 + 공원" 코스가 나온다. 활성화를 기다리는 동안 이걸 한다.
...
3. **`POST /api/recommendations/:id/feedback`** — 개인 단위. 저장/건너뛰기/방문/만족도.
4. **공유 UX** — A가 코스를 뽑으면 **B의 홈에 뜬다.** 요청한 사람만 반응하면
   점수식의 `min(A, B)`를 영원히 못 배운다 (#4, #18).
5. **취향 온보딩 (2지선다)** — 카카오로 축이 생긴 뒤. Q3·Q14.
6. **취향 구조화** — `preferences`(자유 텍스트) → `preference_signals`.
...
친구 2~3커플에게 "실험 중"으로 연다. 그 전에 2·3·4번이 돌아야 한다.
